...
class TestPictureSource(private val gson: Gson) : PicturesSource {
    ...
    override fun getPicturesBetweenDates(fromDate: Date, toDate: Date): Deferred<List<Picture>> =
        CoroutineScope(Dispatchers.IO).async { gson.fromJson<List<Picture>>(json) }
}